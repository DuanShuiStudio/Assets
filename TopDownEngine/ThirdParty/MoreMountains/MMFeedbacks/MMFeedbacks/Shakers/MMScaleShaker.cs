...
{
    /// <summary>
    /// 这个抖动器将允许你移动一个变换组件的缩放比例，既可以是移动一次，也可以是持续移动，使其在指定的持续时间内，在指定的范围内抖动缩放比例。 
    /// 你可以沿着某个方向应用这种抖动效果，无论是否随机化该方向，还可以选择添加噪点和设置衰减效果。 
    /// </summary>
    public class MMScaleShaker : MMShaker
	{
		...
		[Tooltip("这个抖动器的目标应该是变换组件（Transforms）还是矩形变换组件（RectTransforms）。 ")]
		...
		[Tooltip("要抖动其缩放比例的变换组件。如果留空，此组件将以它所附加的变换组件为目标。 ")]
		...
		[Tooltip("要抖动其缩放比例的矩形变换组件。如果留空，此组件将以它所附加的变换组件为目标。 ")]
		...
		[Tooltip("如果这为真，那么将不会使用“抖动主方向”作为抖动的方向，而是会生成一个随机的三维向量，该向量在“抖动主方向”和“抖动备用方向”之间随机取值。 ")]
		public bool RandomizeDirection = false;
		/// when in RandomizeDirection mode, a vector against which to randomize the main direction
		[Tooltip("当处于“随机化方向”模式时，一个用于对主方向进行随机化处理的向量。 ")]
		...
		[Tooltip("当添加方向噪声时，噪声强度将在此值与“方向噪声强度最大值”之间随机确定。 ")]
		...
		[Tooltip("当添加方向噪声时，噪声强度将在此值与“方向噪声强度最小值”之间随机确定。 ")]
		[MMCondition("AddDirectionalNoise", true)]
		...
		[Tooltip("是否使用衰减，衰减会沿着已定义的曲线影响抖动的幅度。 ")]
