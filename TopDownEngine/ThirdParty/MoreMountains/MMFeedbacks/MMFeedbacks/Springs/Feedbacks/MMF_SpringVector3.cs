...
namespace MoreMountains.Feedbacks
{
	...
	/// </summary>
	[AddComponentMenu("")]
	[FeedbackHelp("一种用于操控三维向量（Vector3）弹簧的反馈机制。")]
	...
	public class MMF_SpringVector3 : MMF_Feedback
	{
		...
		[Tooltip("我们想要通过此反馈来操控的三维向量（Vector3）弹簧。如果你设置了一个，那么只有该弹簧会成为目标。如果你不设置，将会向所有与通道数据信息匹配的三维向量弹簧发送一个事件。")]
		...
		[Tooltip("在调用“移动到随机位置（MoveToRandom）”时，用于从中选取随机目标 x 值的最小值（即随机目标 x 值取值范围的下限值）")]
		...
		[Tooltip("调用“移动到随机位置（MoveToRandom）”时，用于从中选取随机目标 y 值的最小值（x 方向的最小值）和最大值（y 方向的最大值）。  （原句中对“min (x)”和“max (y)”的表述从常规逻辑上看有些奇怪，不过已按原文翻译） ")]
		[MMEnumCondition("Command", (int)SpringCommands.MoveToRandom)]
		...
		[Tooltip("调用“随机碰撞（BumpRandom）”时，用于选取随机碰撞 x 值的最小值（x 方向的最小值）和最大值（y 方向的最大值）。 ")]
		...
		[Tooltip("在调用“随机碰撞（BumpRandom）”时，用于从中选取随机碰撞 y 值的最小值（x 方向的最小值）和最大值（y 方向的最大值）。 ")]
		[MMEnumCondition("Command", (int)SpringCommands.BumpRandom)]
