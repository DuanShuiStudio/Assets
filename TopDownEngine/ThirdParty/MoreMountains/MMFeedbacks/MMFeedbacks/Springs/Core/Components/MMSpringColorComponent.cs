...
namespace MoreMountains.Feedbacks
{
    ...
    public abstract class MMSpringColorComponent<T> : MMSpringComponentBase, MMEventListener<MMSpringColorEvent> where T:Component
	{
		...
		[Tooltip("是要监听由一个整数定义的通道，还是监听由一个MMChannel可编写脚本对象定义的通道。整数设置起来很简单，但可能会变得混乱，并且更难记住哪个整数对应着什么。  " +
		...
		[Tooltip("用于监听事件的 MMChannel 定义资源。针对此震动器的反馈必须引用相同的 MMChannel 定义才能接收事件。" +
		...
		[Tooltip("在使这个颜色弹簧产生碰撞效果时应用的乘数（如果你在碰撞时没有获得足够的碰撞颜色，可增大此数值） ")]
		...
		[Tooltip("当在该弹簧的检查器中与任何“移动至”调试按钮进行交互时，要将此弹簧移动到的值。 ")]
		...
		[Tooltip("当在该弹簧的检查器中与“碰撞”调试按钮进行交互时，使该弹簧产生碰撞效果的量。 ")]
