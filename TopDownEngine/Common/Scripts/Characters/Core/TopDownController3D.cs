...
namespace MoreMountains.TopDownEngine
{
    ...
    /// <summary>
    /// 一个控制器，最初改编自Unity的CharacterMotor.js，添加到Unity的原生CharacterController之上，这将处理
    /// </summary>
    public class TopDownController3D : TopDownController
	{
        /// 计算接地状态的可能模式。只有在地面平坦的情况下才应该使用Simple
        public enum GroundedComputationModes { Simple, Advanced }
        
		/// 将当前输入发送给角色
		[MMReadOnly]
		[Tooltip("当前发送给这个角色的输入")]
		...
		[Tooltip("移动计算应该在Update还是FixedUpdate中进行。推荐选择FixedUpdate。")]
		...
		[Tooltip("当从移动的地面上跳跃时，速度应该如何受到影响")]
		...
		[Tooltip("距离地面的距离，超过这个距离角色就不再被认为是接地了")]
		...
		[Tooltip("计算接地状态的选择模式。简单模式应该仅在地面平坦且均匀时使用")]
